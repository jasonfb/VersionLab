RSpec.shared_context "seeded ad platforms" do
  before do
    return if AdPlatform.exists?
    ...
    }.each_with_index do |(platform_name, sizes), platform_index|
      platform = AdPlatform.create!(name: platform_name, position: platform_index)
      sizes.each_with_index do |size, size_index|
        platform.ad_platform_sizes.create!(
          name: size[:name],
          width: size[:width],
          height: size[:height],
          position: size_index
